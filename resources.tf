...
locals {
  ...
  sorted_subnets = sort(data.tfe_outputs.networking.nonsensitive_values.public_subnets)
}
...
resource "aws_instance" "main" {
  count         = length(local.sorted_subnets)
  ami           = nonsensitive(data.aws_ssm_parameter.amzn2_linux.value)
  instance_type = var.instance_type
  subnet_id     = local.sorted_subnets[count.index]
